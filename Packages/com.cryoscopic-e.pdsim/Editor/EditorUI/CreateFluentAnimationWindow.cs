...
namespace PDSim.Editor.EditorUI
{
    ...
    /// </summary>
    public class CreateFluentAnimationWindow : EditorWindow
    {
        public VisualTreeAsset animationAttributeTemplate;

        private PdSimFluent _metadata;

        private ScrollView _animationAttributeList;

        private PdSimFluentAnimation _context;

        public static void ShowAsModal(PdSimFluent metadata, PdSimFluentAnimation context)
        {
            var wnd = GetWindow<CreateFluentAnimationWindow>();
            wnd.titleContent = new GUIContent("Create New Fluent Animation");
        ...
        public void UpdateContent()
        {
            ...
            var animationName = root.Q<Label>("Name");
            animationName.text = _metadata.ToString();

            // Types list

            _animationAttributeList = root.Q<ScrollView>("TypesList");
            ...
            {
                var controller = new AnimationAttributeController();
                var fromUxml = animationAttributeTemplate.Instantiate();
                controller.SetVisualElement(fromUxml);
                controller.SetMetadata(item);
                controller.UpdateContent();
                _animationAttributeList.Add(fromUxml);
            }
        ...
        private void CreateAnimation()
        {
            ...
            foreach (var item in _animationAttributeList.Children())
            {
